...
preflight(){
    output "Pterodactyl Installatie en Upgrade Script"
    output "Copyright © 2020 Devserge <huijsenserge@gmail.com>."
    output "Dit is een fork van een oude versie van Tommy Tran, deze is nieuwer en veiliger."
    output ""

    output "Let erop dat je dit script uitvoert op een Fresh OS."
    output "Ik detecteer even op welk systeem je draait.."

    os_check

    if [ "$EUID" -ne 0 ]; then
        output "Voer dit script a.u.b. uit als root!"
        exit 3
    fi

    output "Automatische de architecture  vinden.."
    MACHINE_TYPE=`uname -m`
    if [ ${MACHINE_TYPE} == 'x86_64' ]; then
        output "64-bit server gevonden! We are Good to go."
        output ""
    else
        output "Unspported architecture gevonden. Switch naar 64-bit (x86_64)."
        exit 4
    fi

    output "Automatisch virtualisatie methode detecteren.."
    ...
    if [ "$virt_serv" = "" ]; then
        output "Virtualisatie: Bare Metal of KVM gevonden."
    elif [ "$virt_serv" = "openvz lxc" ]; then
        output "Virtualisatie: OpenVZ 7 gevonden. Niet optimaal."
    elif [ "$virt_serv" = "xen xen-hvm" ]; then
        output "Virtualisatie: Xen-HVM gevonden."
    elif [ "$virt_serv" = "xen xen-hvm aws" ]; then
        output "Virtualisatie: Xen-HVM op AWS gevonden."
        warn "Als je allocations toevoegt moet dit via de google AWS cloud service gaan"
        warn "Resuming in 10 seconds..."
        sleep 10
    else
        output "Virtualisatie: $virt_serv gevonden."
    fi
    output ""
...
install_options(){
    output "Selecteer je installatie opties:"
    output "[1] Installeer het panel."
    output "[2] Installeer de daemon."
    output "[3] IInstalleer Standalone SFTP server"
    output "[5] Upgrade 0.7.x panel naar 0.7.18."
    output "[6] Upgrade 0.6.x daemon naar 0.6.13."
    output "[7] Upgrade het panel naar 0.7.18 en de daemon naar 0.6.13"
    output "[8] Upgrade de standalone SFTP naar 1.0.5."
    output "[9] Pterodactyl compatible maken met de Pterodactyl APP"
    output "[10] Update mobile compatibility."
    output "[11] Installeer of update phpmyadmin 5.0.2."
    output "[12] Installeer een standalone database host."
    output "[13] Veranderd Pterodactyl theme."
...
webserver_options() {
    output "Welke soort webserver wil je erbij installeren \n[1] Nginx (aangeraden).\n[2] Apache2/httpd."
    read choice
    case $choice in
        1 ) webserver=1
...
theme_options() {
    output "WWil je er een leuk thema bij installeren?"
    output "[1] Nee dankje."
...
required_infos() {
    output "Vul je e-mailadres in:"
    read email
    dns_check
}

dns_check(){
    output "Vul je server domeinnaam in (geen IP!):"
    read FQDN
    ...
    if [ "${SERVER_IP}" != "${DOMAIN_RECORD}" ]; then
        ...
        output "Je domein klopt, good to go!"
    fi
}
...
block_icmp(){
    output "Wil je ICMP (Ping) pakketten blokkeren?"
    output "Kies [1] als je gebruikt maak van monitoring. Dit zorgt ervoor dat je NIET kan pingen naar de server."
    output "[1] Ja."
    output "[2] Nee."
    read icmp
    case $icmp in
        1 ) /sbin/iptables -t mangle -A PREROUTING -p icmp -j DROP
            ...
            ;;
        2 ) output "Overslaan van rule..."
            ;;
        * ) output "Geen geldige invoer."
            block_icmp
    esac
}

javapipe_kernel(){
    output "Wil je JavaPipe software DDoS Protection erbij installeren? (https://javapipe.com/blog/iptables-ddos-protection)?"
    output "[1] Ja."
    output "[2] Nee."
    read javapipe
    case $javapipe in
        1)  sh -c "$(curl -sSL https://raw.githubusercontent.com/tommytran732/Anti-DDOS-Iptables/master/javapipe_kernel.sh)"
            ;;
        2)  output "javaPipe wordt niet geinstalleerd"
            ;;
        * ) output "Geen geldige invoer."
            javapipe_kernel
    esac
...
install_database() {
    ...
    output "Database aanmaken met login.."
    password=`cat /dev/urandom | tr -dc 'a-zA-Z0-9' | fold -w 32 | head -n 1`
    adminpassword=`cat /dev/urandom | tr -dc 'a-zA-Z0-9' | fold -w 32 | head -n 1`
    rootpassword=`cat /dev/urandom | tr -dc 'a-zA-Z0-9' | fold -w 32 | head -n 1`
    Q0="DROP DATABASE IF EXISTS test;"
    ...
    output "MariaDB binden aan 0.0.0.0."
	if [ -f /etc/mysql/my.cnf ] ; then
        sed -i -- 's/bind-address/# bind-address/g' /etc/mysql/my.cnf
		sed -i '/\[mysqld\]/a bind-address = 0.0.0.0' /etc/mysql/my.cnf
		output 'Restarting MySQL process...'
...
broadcast(){
    ...
    output "###############################################################"
    output "FIREWALL INFORMATIE"
    output ""
    output "Ale onnodige ports zijn standaard geblokkeerd."
    if [ "$lsb_dist" =  "ubuntu" ] || [ "$lsb_dist" =  "debian" ]; then
        output "Gebruik 'ufw allow <port>' om een poort toe te voegen"
    elif [ "$lsb_dist" =  "fedora" ] || [ "$lsb_dist" =  "centos" ] && [ "$dist_version" != "8" ]; then
        output "Gebruik 'firewall-cmd --permanent --add-port=<port>/tcp' om een poort toe te voegen."
    fi
    output "###############################################################"
    output ""
}
...
broadcast_database(){
        output "###############################################################"
        output "MariaDB / MySQL informatie"
        output ""
        output "Je root password is $rootpassword"
        output ""
        output "Je externe MariaDB host voor Pterodactyl is"
        output "Host: $SERVER_IP"
        output "Port: 3306"
        output "User: admin"
        output "Wachtwoord: $adminpassword"
        output "###############################################################"
        output ""
}
